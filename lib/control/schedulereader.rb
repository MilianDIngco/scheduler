...
class ScheduleReader
	...
	def self.readCSV(file_paths, required_headers)
		buildings = {}
		reservations = {}	
		...
		reservation_list_csv.each do |row|
			...
			if (Integer(row["Room"]) rescue false) == false
				puts "ERROR: Invalid room value input"
				next
			end

			if (Integer(row["Room"]) < 0)
				puts "ERROR: Negative room value"
				next
			end
		...
		room_list_csv.each do |row|
			...
			room_capacity = Integer(row["Room"])
			room_res_list = nil
			...
			temp_room = Room.new(row, room_no, room_capacity, res_list)
	...
	def self.valid_room (row, valid_values) 
		...
		if ((Integer(room_num) rescue false) != false) 
			if (Integer(room_num) < 0)
				puts "Negative room number: #{room_num}"
				return false
			end
		else
			puts "Invalid room number: #{room_num}"
			return false
		end 
		...
		if ((Integer(capacity) rescue false) != false)
			if (Integer(capacity) < 0)
				puts "Negative capacity: #{capacity}"
				return false
			end
		else
		...
		row.each do |header, value|
			...
			if valid_values.key?(header) 
				...
				if !(valid_values[header].include?(value))
					puts "Failed check header: #{header} value: #{value}"
					return false
				end
			end
		end
